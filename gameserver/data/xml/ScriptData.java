...
import l2j.luceraV3.gameserver.scripting.ScheduledQuest;
...
public final class ScriptData implements IXmlReader, Runnable
{
	...
	public void parseDocument(Document doc, Path p)
	{
		forEach(doc, "list", listNode -> forEach(listNode, "script", scriptNode ->
		{
			...
			{
				// Create the script.
				Quest instance = (Quest) Class.forName("l2j.luceraV3.gameserver.scripting." + path).getDeclaredConstructor().newInstance();
